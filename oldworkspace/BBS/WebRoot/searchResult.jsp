...
<% 
	...
	try {
		...
		rsCount = DB.executeQuery(stmtCount,
				"select count(*) from article where pid = 0 and (title like '%" + keyword + "%' or cont like '%"+ keyword +"%')");
		rsCount.next();
		...
		String sql = "select * from article where pid = 0 and (title like '%" + keyword + "%' or cont like '%" + keyword + "%') order by pdate desc limit "
				+ startPos + "," + PAGE_SIZE;
		stmt = DB.createStmt(conn);
		rs = DB.executeQuery(stmt, sql);
		if(rs == null) {
		out.println("找不到包含关键词" + keyword + "的帖子");
		return;
		}
...
<body>
	...
	<div id="jive-forumpage">
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
			<tbody>
				<tr valign="top">
					<td width="98%"><p class="jive-breadcrumbs">论坛: 简易论坛 搜索结果</p>
				...
				<tr valign="top">
					<td><span class="nobreak"> 第<%=pageNo%>页，共<%=totalPages%>页
							- <span class="jive-paginator"> [ <a
								href="searchResult.jsp?pageNo=1&keyword=<%= keyword %>">第一页</a> | <a
								href="searchResult.jsp?pageNo=<%=pageNo == 1 ? 1 : pageNo - 1%>&keyword=<%= keyword %>">上一页</a>
								| <a
								href="searchResult.jsp?pageNo=<%=pageNo == totalPages ? pageNo : pageNo + 1%>&keyword=<%= keyword %>">下一页</a>
								| <a href="searchResult.jsp?pageNo=<%=totalPages%>&keyword=<%= keyword %>">最后一页</a> ]
						</span>
					</span></td>
									...
									<tbody>
										...
										<tr
											...
											<td nowrap="nowrap" width="1%">
												...
												%> <a href="update.jsp?id=<%=a.getId()%>&from=<%=url%>">更新</a>
												<a
