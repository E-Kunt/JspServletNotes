...
<%
	...
	if (action != null && action.trim().equals("login")) {
		...
		if(user == null || !user.trim().equals("admin")) {
			out.println("<script type='text/javascript'>alert('用户名错误！');</script>");
		} else if(password == null || !password.trim().equals("admin")) {
			out.println("<script type='text/javascript'>alert('密码错误！');</script>");
		} else {
			session.setAttribute("isAdmin", "true");
			out.println("<script type='text/javascript'>alert('登录成功！'); window.top.location.href='article.jsp';</script>");
		}
	}
%>
...
<BODY>
	...
	<DIV
		...
		<P style="padding: 30px 0px 10px; position: relative;">
			<SPAN class="u_logo"></SPAN> <INPUT class="ipt" type="text"
				placeholder="请输入用户名" name="user" value="<%=user%>">
		</P>
		...
		<DIV
			...
			<P style="margin: 0px 35px 20px 45px;">
				<SPAN style="float: right;"><A
					style="color: rgb(204, 204, 204); margin-right: 10px;" href="article.jsp">游客进入</A>
	...
	<div style="text-align:center;">
		<p>
			<a href="#" target="_blank">简易论坛|开发者：王艺焜</a>
		</p>
	</div>
	</form>
</BODY>
